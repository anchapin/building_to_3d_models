...
# Deployment script for Building-to-3D Conversion Application
...
cp -r frontend/building-to-3d-app/build/* $DEPLOY_DIR/
...
cat > $DEPLOY_DIR/api/requirements.txt << 'EOF'
flask==2.0.1
flask-cors==3.0.10
werkzeug==2.0.1
numpy==1.21.0
...
cd $DEPLOY_DIR
zip -r ../building_to_3d_app.zip ./*
cd ..

echo "Deployment package created: building_to_3d_app.zip"
echo "To deploy, extract the zip file and run:"
echo "  cd api && pip install -r requirements.txt && python app.py"
